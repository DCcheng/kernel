...
class VideoCommand
{
    ...
    //检查视频文件是否存在B帧以及是否H264编码
    public static function checkVideoTranscode($filename)
    {
        $isExistBFrames = false;
        $isH264 = false;
        $info = [];
        Kernel::command()->addCommand([
            'sudo ffprobe ' . $filename . ' -show_streams -read_intervals %+0.5 -v quiet -show_frames -select_streams v|grep -E "pict_type=B|codec_name=h264"'
        ])->execute(false, "/usr/bin", $info);
        $len = count($info);
        if ($len > 0) {
            if ($info[0] == "pict_type=B") {
                $isExistBFrames = true;
            }
            if ($info[$len - 1] != "codec_name=h264") {
                $isH264 = true;
            }
        }
        return [$isExistBFrames, $isH264];
    }

    //开启rtsp转flv直播
    public static function rtspToRtmp($url, $targetUrl)
    {
        $info = [];
        Kernel::command()->addCommand([
            "ps -aux|grep \"ffmpeg -rtsp_transport tcp -rtbufsize 50M -i " . $url . " -vcodec copy -acodec copy -f flv\" | grep -v grep "
        ])->execute(false, "/usr/bin", $info);
        if (count($info) <= 0) {
            Kernel::command()->addCommand([
                "nohup ffmpeg -rtsp_transport tcp -rtbufsize 50M -i " . $url . " -vcodec copy -acodec copy -f flv " . $targetUrl . " 1>/dev/null 2>&1 &"
            ])->execute(false, "/usr/bin");
        }
    }

    //关闭rtsp转flv直播
    public static function killRtspToRtmp($url)
    {
        Kernel::command()->addCommand([
            "sudo ps -aux|sudo grep \"ffmpeg -rtsp_transport tcp -rtbufsize 50M -i " . $url . " -vcodec copy -acodec copy -f flv\"|sudo awk '{print $2}'|sudo xargs sudo kill -9"
        ])->execute(false, "/usr/bin");
    }

    //转码
    public static function transcode($path, $source, $target, $binPath = "usr/bin")
    {
        Kernel::command()->addCommand([
            "nice ffmpeg -y -i " . $path . "/" . $source . " -acodec aac -c:v libx264 -r:v 25 -g 25 -profile baseline -f mp4 " . $path . "/" . $target,
        ])->execute(true, $binPath);
    }
}
