...
class Response
{
     ...
     * @param int $code
     */
    public function fail($message,$code = 404){
        $result = ["message" => $message,"status_code" => $code];
    ...
    public function response($result,$code = 200){
        try {
            header(self::$code[$code]);
            echo json_encode($result);
            exit();
        }catch (Exception $exception){
            header(self::$code[404]);
            echo json_encode(["message" => "返回状态码缺失","status_code" => 404]);
            exit();
        }
    }
}
